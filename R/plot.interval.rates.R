...
# function to plot the output from independent methods: x-axis = absolute time, multiple options for plotting the the proportion of useful replicates
#' @export
#'
#'
plot.p.q.est.ab<-function(in1,in2,title="Title",col="grey75",p.true=lambda,q.true=mu,psv=F,in3,abs=F,trees,upp=1){
  ...
  # if ab = T, use absolute not relative time -- presently inactive ????
  if(abs){
    trees<-trees
  }
  upp<-upp # options = 1,2,3
  # -------
  if (col=="grey75"){
    col1="grey75"
    col2="red"
  ...
  # calculate the mean
  # calculate the standard deviation #???? I think this is now the 95% HPDs
  #s=round(sd(t.rates$p.total,na.rm=T),2)
  #n=length(t.rates$p.total)
  if(length(valid) > 1){
    m=round(mean(t.rates$p.total,na.rm=T),2)
    hpds=HPDinterval(as.mcmc(t.rates$p.total))
  ...
  if(length(valid) > 1){
    ...
    hpds=HPDinterval(as.mcmc(t.rates$q.total))
    lower=round(hpds[1],2)
    upper=round(hpds[2],2)
  ...
  plot.polygons(polygons.p,col=col) # add color option
  ...
  if(upp==1){
    ...
    #plot(df$h, df$mean, ylim = c(0,1), xlim=c(0,max(df$h)), type="n",xlab="",ylab="% URs",bty="n",xaxt="n",tcl=-0.2) #???? delete
    plot(df$h, df$mean, ylim = c(0,1), xlim=c(0,max(df$h)), type="n",xlab=NA,ylab=NA,bty="n",xaxt="n",yaxt="n")
    lines(df$h,y=df$use,col="darkgrey",lwd=3,lty=1)
  ...
  #bam=max(unlist(lapply(1:100,function(x){ max(n.ages(trees[[x]])) } )))
  #basin.age.max=round(bam,1)+0.1
  #if(basin.age.max < 10){
  ba.mx=round_any(basin.age.max,5,ceiling)
  #}
  #else{
  #  ba.mx=round_any(basin.age.max,20,ceiling)
  #}
  ...
  plot.polygons(polygons.q,col=col) # add color option
  # useful proportion
  if(upp==1){
    lines(df$h,y=df$use,col="darkgrey",lwd=3,lty=1)
  }
  ...
  # define the maximum limit on the x-axis
  # what's going on here? ???? delete? how *does the fxn" calculate the max?
  #basin.age.max=round(max(trees[[2]]),1)+0.1 # define the maximum basin age
  #if(basin.age.max < 10){
  #  ba.mx=round_any(basin.age.max,10,ceiling)
  #}
  #else{
  #  ba.mx=round_any(basin.age.max,20,ceiling)
  #}
  # define an arbitrary set of horizon ages
  #horizon.ages.arb=seq(from=ba.mx,to=0,length.out=20)
  # initiate the plot -- note df$mean here is not meaningful
...
# plot.p.q.est.ab
#
# #plot.p.q.est = alternative
#
# use :: for coda functions
# polygon.subsets
# experiment with the upp and write documentation
# plot.polygons
# tree.reps.ps
